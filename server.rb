...
def checkQuery(cube, start, queryDim, method)
            ...
            if(queryDim.all?{|levels| levels[0] == dimension })
                ...
                #puts erstSn.inspect
                #puts "-----"
                #Ersten beiden gleich
                if(erster[0] == zweiter[0])
                    #puts "\t\tErsten beiden gleich"
                    #puts method.to_sym
                    #start = start.send(method.to_sym,'[#{erster[1]}].[#{erster[1]}]#{erstSn}','[#{zweiter[1]}].[#{zweiter[1]}]#{zweitSn}')
                    if(!dritter.nil?)
                        #start = start.hierarchize.crossjoin('[#{dritter[0][1]}].[#{dritter[0][1]}]#{drittSn}')
                    end
                end
                #Letzten beiden gleich
                if(erster[0] != zweiter[0])
                   start = start.send(method.to_sym,"[#{erster[1]}].[#{erster[2]}]#{erstSn}")

                    if(!dritter.nil? && zweiter[0] == dritter[0])
                        #puts "\t\tdrei vorhanden"
                        #puts "\t\tLetzten beiden gleich"
                        ...
                        start = start.crossjoin("[#{zweiter[1]}].[#{zweiter[2]}]#{zweitSn}").hierarchize.crossjoin("[#{dritter[1]}].[#{dritter[2]}]#{drittSn}").hierarchize
                         ...
                         #puts "\t\tnur zwei vorhanden -- beide verschieden"
